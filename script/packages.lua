...
package("ffmpeg")
    ...
    add_configs("pic",              {description = "Build shared library with position independent code.", default = false, type = "boolean"})
    ...
    add_configs("d3d11va",          {description = "Enable d3d11va hardware acceleration.", default = false, type = "boolean"})
    add_configs("dxva2",            {description = "Enable dxva2 hardware acceleration.", default = false, type = "boolean"})
    add_configs("mediafoundation",  {description = "Enable mediafoundation hardware acceleration.", default = false, type = "boolean"})
    add_configs("user_config",      {description = "Use user config (disables test!)", default = {}, type = "table"})
    ...
    on_install("windows", "mingw@windows,linux,cygwin,msys", "linux", "macosx", "android", "iphoneos", function (package)
        ...
        for _, user_config in pairs(package:config("user_config")) do
            table.insert(configs, user_config)
        end
        for name, enabled in table.orderpairs(package:configs()) do
            if type(enabled) == "boolean" and not package:extraconf("configs", name, "builtin")then
                if enabled then
        ...
        if package:is_plat("windows") then
            ...
            local mingw = package:toolchain("mingw") or toolchain.load("mingw", {plat = package:plat(), arch = package:arch()})
            assert(mingw:check(), "mingw not found! please install mingw and add it to the PATH environment variable.")
            ...
            if pkg_config_path then
                local paths = os.joinenvs(os.getenvs(), mingw:runenvs())
...
package("mongoose")
    ...
    add_configs("ssl", {description = "Enable SSL", default = "builtin", type = "string", values = {"openssl", "mbedtls", "builtin"}})
    add_configs("io_size", {description = "IO buffer size", default = 8192, type = "number"})
    add_configs("ssi", {description = "Enable SSI", default = false, type = "boolean"})
    ...
    on_install(function (package)
        os.cp(path.join(os.scriptdir(), "mongoose.lua"), "xmake.lua")
